...
const Header = ({ className }) => {
  const data = useStaticQuery(
    graphql`
      query {
        desktop: file(relativePath: { eq: "hero-bg@2x.png" }) {
          childImageSharp {
    ...
    >
      <div className={`header-content m-auto text-center max-w-full pt-12 sm:pt-20 pb-4 md:pb-0 px-4`} >
        <div className={`relative z-20`}>
          <h1 className={`font-semibold text-xl sm:text-xxl md:text-xxxl tracking-tight text-dark dark:text-white`} >
            Device Mockups
          </h1>
          <p className={`mb-6 text-base sm:text-md md:text-lg text-gray-700 dark:text-gray-300 m-auto max-w-lg md:max-w-xl`}>
          ...
          <div className={`z-10 hidden sm:block mt-16 opacity-50 text-gray-900 dark:text-white`}>
